...
class c_settings extends Admin{
    ...
    public function save(){
		...
		foreach($settings as $key => $value){
			...
			$value = addslashes(str_replace('"', "'", $value)); //将引双引号替换成单引号, 再加上反斜杠
    ...
    public function index(){
		...
		TableRow(array('<B>客服系统URL</B><BR><font class=grey>WeLive在线客服系统安装后的完整URL, 用于找回密码、邮件发送等. 请以 <span class=note>/</span> 结束.<BR>将: <span class=note>&lt;script type="text/javascript" charset="UTF-8" src="' . BASEURL . 'welive.js"&gt;&lt;/script&gt;</span><BR>插入网页代码的&lt;html&gt;内, 才能加载(显示)WeLive客服小面板, 此面板为访客提供对话窗口.</font>', '<input type="text" style="width:292px;" name="settings[BaseUrl]" value="' . BASEURL . '">'));
		...
		TableRow(array('<B>访客窗口颜色风格</B><BR><font class=grey>WeLive在线客服系统前台访客对话窗口的颜色风格.</font>', $Radio->Get()));
		...
		TableRow(array('<B>保存对话记录</B><BR><font class=grey>将访客与客服间的对话记录保存到MySQL数据库中. 如果追求 <span class=note>更快的交互性能</span>, 可以关闭保存记录的功能.<BR>注: 此项设置仅在Socket服务重启后生效, 进入客服后可以重启Socket服务.</font>', $Radio->Get()));
		...
		TableRow(array('<B>访客上传授权</B><BR><font class=grey>访客登录WeLive客服系统后, 是否需要经过客服人员授权后才能上传图片文件等. <br><span class=note>如果设置为“不需要”, 即访客可自由上传, 将可能大量导致垃圾文件上传到服务器</span>.</font>', $Radio->Get()));
		...
		TableRow(array('<B>欢迎信息(<span class=blue>中文</span>)</B><BR><font class=grey>访客(中文浏览器)进入客服后，首次显示的问候语.</font>', '<textarea style="width:400px;height:60px;" name="settings[Welcome]">' . APP::$_CFG['Welcome'] . '</textarea>'));

		TableRow(array('<B>欢迎信息(<span class=red>English</span>)</B><BR><font class=grey>访客(非中文浏览器)进入客服后，首次显示的问候语.</font>', '<textarea style="width:400px;height:60px;" name="settings[Welcome_en]">' . APP::$_CFG['Welcome_en'] . '</textarea>'));


		TableRow(array('<B>留言提示信息(<span class=blue>中文</span>)</B><BR><font class=grey>客服离线时, 显示在留言板上的提示信息.</font>', '<textarea style="width:460px;height:60px;" name="settings[Comment_note]">' . APP::$_CFG['Comment_note'] . '</textarea>'));

		TableRow(array('<B>留言提示信息(<span class=red>English</span>)</B><BR><font class=grey>客服离线时, 显示在留言板上的英文提示信息.</font>', '<textarea style="width:460px;height:60px;" name="settings[Comment_note_en]">' . APP::$_CFG['Comment_note_en'] . '</textarea>'));
