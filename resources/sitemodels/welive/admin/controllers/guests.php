...
class c_guests extends Admin{
	...
	public function save(){
		...
		if(!IsEmail($email)) Error('Email地址不规范', '编辑客人错误');
	...
	public function edit(){
		...
		TableHeader('编辑客人信息: <span class=note>' . Iif($user['fullname'], $user['fullname'], Iif($user['lang'], "无名 (ID: $gid)", "No Name (ID: $gid)")) . '</span>');
	...
	public function index(){
		...
		if($maxrows['value'] < 1){
			...
			while($user = APP::$DB->fetch($getguests)){
				...
				'<a title="编辑" href="'.BURL('guests/edit?gid='.$user['gid']).'">' . Iif($user['fullname'], $user['fullname'], '<font class=grey>' . Iif($user['lang'], '无名', 'No Name') . '</font>') . '</a>',
				$user['grade'],
				Iif($user['lang'], '中文', 'EN'),
				$user['logins'],
				$user['banned'],
