...
function WeLiveWin() {
	this.Create = function(id, title, wbody, lang, au){
		...
		$(winbody).children(".g_bott").children(".g_msg").keyup(function(e){if(e.keyCode ==13) guest_send();}); //输入框enter发送
		...
		g_tools.children(".t_phrase").tipTip({content: (lang == 1 ? $(".phrases_div").html().replace(/towhere/ig, id) : $(".phrasesen_div").html().replace(/towhere/ig, id)), keepAlive:true, maxWidth:"348px", defaultPosition:"top", left: -53, arrowLeft: -123, edgeOffset:0, delay:400, parent: true, hoverClass: "hover"});
...
function guest_authupload(){
	guest_output('免费版无此功能!', CurrentId, 3);
...
function welive_init(){
	...
	$(".sysinfo>a").tipTip({content: '1. 按 Ctrl + Alt: 在客服交流区与当前客人小窗口间切换.<br>2. 按 Ctrl + 下箭头 或 Esc键: 关闭当前客人小窗口.<br>3. 按 Ctrl + 上箭头: 展开关闭的客人小窗口.<br>4. 按 Ctrl + 左或右箭头: 在已展开的客人小窗口间切换.<br>5. 客人被踢出或禁言后, 其刷新页面仍可重新进入客服.<br>6. 客人获取上传授权后将会一直保留, 直至被客服解除.', maxWidth:"400px", defaultPosition:"top", delay:300});
	...
	//管理员专有
	if(admin.type == 1){
		$(".set_robot_on").tipTip({content: '1. 开启无人值守后, 新上线的访客将由机器人自动回复, 已在<br>&nbsp;&nbsp;&nbsp;&nbsp;线的访客仍由原客服提供服务.<br>2. 开启无人值守后, 所有客服均不再接受新客人, 且可离线.<br>3. 开启无人值守后, 访客留言功能将失效.<br>4. 只有关闭无人值守, 新登录的客服才能提供在线服务.<br>5. Socket服务重启后, 无人值守状态需要重新设置.'});
...
//定义全局变量
var tttt = 0, ttt_1 = 0, pagetitle, flashtitle_step = 0, sounder, towhere = 0;
