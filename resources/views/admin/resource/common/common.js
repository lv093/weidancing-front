var serverUrl = "http://127.0.0.1:10006";//线上
var serverUrlBackend = "/backend";//线上
var serverUrl = "";
...
//获取轮播图全部列表
var getAllBannerLists = serverUrl + '/banner/getAllList?token=' + getUserToken();
//搜索轮播图
...
function msgSuccess(result,time){
    var t = isBlank(time) ? 800 :time;
    layer.msg(result, {
        icon: 1,
        time:t,
        shade: 0.01
...
function uploadImageIcon() {
    ...
    iconView = "#iconView";
    $("#uploadFormIcon input").click();
}
function uploadAndroidIcon() {
    cover = "#androidDownload";
    photoCover = "#androidDownloadCover";
    iconView = "#androidDownloadView";
    $("#uploadFormIcon input").click();
}
function uploadIosIcon() {
    cover = "#iosDownload";
    photoCover = "#iosDownloadCover";
    iconView = "#iosDownloadView";
    $("#uploadFormIcon input").click();
}

/*--------------重写图片上传后台方法-----------*/
$("#uploadFormIcon input").change(function () {
    $(photoCover).val($(this).val());
    var formData = new FormData($(this).parent()[0]);
    $.ajax({
        url: '/storage/handle', //Server script to process data
...
//关闭标签页
function closeMyselfInIframe(className) {
    if(arguments.length ==0 ){
        var className = '.J_iframe';
    }
    var thisHref = window.location.href,
        $mainBody = $(window.parent.document).find("body");

    $mainBody.find("iframe"+className).each(function () {
        if (this.src  === thisHref) {
...
function MyTips(id, data) {
    var html = "";
    var parse = JsonParse(data);
    if (parse.length > 30) {
        html += '<span style ="cursor:pointer;"  id=' + id + '\ onclick =\'showDesc(' + id + "," + JSON.stringify(data) + ')\' >';
        var str = parse.substring(0,30);
        html += str + "...";
    } else {
        html += parse;
    }
...
function showDesc(id, desc) {
    var html = JsonParse(desc);
    layer.open({
        type: 1,shade: 0.01 ,area: '400px;',shadeClose:true, title: false, //不显示标题
        content: '<div class="wrapper wrapper-content layer_notice">'+html+' </div>',
    });
}